...
Timestamps are used to track when objects are created, updated, and deleted. The native `time.Time` type is used to represent timestamps, and is automatically converted to and from Unix time. PomDB automatically sets the `CreatedAt` and `UpdatedAt` fields when an object is created or updated, and sets the `DeletedAt` field when an object is [soft-deleted](#soft-deletes):
